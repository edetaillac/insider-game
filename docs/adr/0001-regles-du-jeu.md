...
D6 (21/09/2026, gameplay). Révélation du mot sans tell. Au livret, tout le monde ferme les yeux et seul le Traître regarde : personne ne sait qui a regardé. En numérique, si seuls le Maître et le Traître ont une carte à retourner, celui qui touche son téléphone est le Traître. Décision : en phase "mot", tout le monde a la même carte face cachée ; le Maître et le Traître y lisent le mot, le Citoyen une phrase neutre de longueur comparable ; la carte se recache seule après une durée fixe identique pour tous (5 s), pas au tap. Même auto-masquage sur la carte de rôle par cohérence. Pendant le jeu, seul le Maître garde le mot consultable (son rôle est public), le Traître le retient comme au livret. Implémenté côté client seulement ; si des joueurs oublient de retourner leur carte, passer au niveau 2 : commande "j'ai regardé" dans le moteur et chrono lançable seulement quand tout le monde a regardé. Niveau 2 réalisé le 21/09 : accusés `seenRole` et `seenWord` dans le moteur, progression collective affichée. Le chrono reste lançable par l'hôte sans attendre tout le monde. Correctif du 21/09 : l'accusé passe par un tap explicite ("J'ai vu ma carte", "J'ai regardé"), inactif tant que la carte n'a pas été retournée au moins une fois, dans les deux phases. ~~Pendant l'enquête, le Traître comme le Maître gardent le mot consultable dans un bloc sombre (la vue l'expose déjà aux deux).~~ Annulé le 24/09 (voir D7) : le Traître lit le mot une seule fois, au rituel. Pendant l'enquête, seul le Maître garde le mot.

D7 (24/09/2026, gameplay, retour au livret et à la version initiale de D6). Le Traître lit le mot une seule fois, au rituel du mot. Ensuite il ne l'a plus : s'il l'oublie, c'est le jeu. Les téléphones restent posés cinq minutes, un bloc visible chez le Traître seul le trahirait à distance. Décision : la vue ne transmet plus le mot au Traître en enquête, discussion et votes (il le retrouve en fin de manche), ce qui est plus sûr qu'un masquage côté client. Citoyens et Traître ont donc exactement le même écran d'enquête. Le Maître garde le mot affiché, son rôle est public. Même logique pour le rôle : on voit sa carte une fois, il n'est plus rappelé pendant l'attente du mot. Une première version (appui maintenu pour revoir le mot) a été abandonnée le jour même
D8 (24/09/2026, gameplay). Seul le Maître déclare le mot trouvé et clôt la discussion, conformément au livret (A-5, B-1). L'hôte ne le fait à sa place que si le Maître est hors ligne. La présence vit dans la table, qui la transmet au moteur (`setMasterAway`) pour que les actions proposées restent justes. Le lancement du chrono reste ouvert au Maître et à l'hôte
